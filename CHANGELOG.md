...
* The **Send HTTP Request** instruction now offers a `$$DESCRIPTION$$` token that will insert the description of the instruction into the request body or URL query string
* Clicking on any message token listed on the **Message Token Help** tab will now select the the whole token text for easy copying via Ctrl+C.
